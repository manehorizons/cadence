 ...
 *
 * Relocated from `cli/commands/next.ts` into `services/next.ts` (phase 221
 * T3) so `mcp/tools.ts` can import it directly — core never imports host
 * code, but MCP is part of core and previously could not reach this
 * `cli/commands/`-only logic. Parameter order normalized to
 * `(repoRoot, args, io)` to match every other `services/*.ts` module; the
 * CLI's own behavior, output text, flags, and exit codes are unchanged.
 */
