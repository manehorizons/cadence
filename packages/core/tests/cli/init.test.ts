...
describe('keel init', () => {
  it('scaffolds .keel/ with team preset by default', async () => {
    active = await tempRepo();
    const r = await run(['init', '--name=demo'], active.root);
    expect(r.code).toBe(0);
    expect(existsSync(join(active.root, '.keel/config.json'))).toBe(true);
  ...
  it('refuses to overwrite existing .keel/', async () => {
