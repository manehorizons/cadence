...
**Deliberately deferred to a named v1.2 "Public release" milestone** (see ROADMAP): the real public-npm publish, npm provenance (requires the source repo be public — a conscious repo-visibility decision), `.github/workflows/release.yml` gated on `ci-success`, changesets adoption, and re-deciding whether `@manehorizons/cadence-testkit` ever publishes. The v1.1 boundary is "prove the path without irreversible action."
