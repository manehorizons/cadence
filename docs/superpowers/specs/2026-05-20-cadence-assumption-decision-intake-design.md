...
9. **No bucket-by-status render** — both shapes are flat. `## Open / ## Validated / ## Rejected` sections ~~wait until the transitions slice exists~~ — shipped in Slice 9 (otherwise the `## Validated` / `## Rejected` sections are dead until status moves off `'open'`).
