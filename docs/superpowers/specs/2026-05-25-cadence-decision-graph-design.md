...
- ~~**Bidirectional `Decision.supersedes: dec-X[]` derived backfill** — mirror Slice 11's `assumptionIds`/`decisionIds` pattern; would let `decision show` surface the inverse link without re-walking the ledger.~~ **SHIPPED Slice 31** (`deriveDecisionInverseLinks` wired into add/transition/reconcile; `decision show` surfaces the bullet). Slice 29 graph viewer NOT optimized to use the new field — current inverse-lookup works on any ledger; using `supersedes[]` directly would couple correctness to field consistency. **CLOSED — won't do (Slice 39 brainstorm, 2026-05-29):** `supersedes[]` is the cached output of the exact scan `walkAncestorTree` runs, there is no measured perf problem, and trusting it would trade robustness for nothing. Full rationale in `2026-05-25-cadence-decision-supersedes-backfill-design.md` § Follow-On. The viewer stays as-is.
