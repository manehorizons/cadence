...
| **manual** | `node`, `initialized`, corrupt `state.json`, `worktree-phases`, `verification-readiness`, `codex-cadence-command`, `ledger-remote-collision`, user-owned prompt/agent files | never auto-applied — reported as guidance with the check's remediation |
