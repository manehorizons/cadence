...
- **v1.5.0 Session continuity** — ⏳ ON `main`, UNRELEASED. `cadence handoff`/`resume` + `/cadence-handoff`/`/cadence-resume` (phase `46-handoff-resume`) and the boundary-check path-normalization fix (phase `47-boundary-path-fix`) are merged to `main`; `package.json` bumped `1.4.0 → 1.5.0` via changesets. **Next:** run the `Release` workflow (`workflow_dispatch`, `dry_run=false`) to publish `1.5.0`, then cut tag `v1.5.0`. Not yet on npm. See MILESTONES.md §v1.5.0.
